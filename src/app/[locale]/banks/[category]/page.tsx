...
export async function generateMetadata({
  ...
  if (!cat || cat.vertical !== "BANKING") return buildMetadata({ locale, noIndex: true });
...
export default async function BankCategoryPage({
  ...
  const cat = await getCategoryBySlug(category);
  if (!cat || cat.vertical !== "BANKING") notFound();
  ...
  return (
    <ProductListing
      ...
      crumbs={[
        { name: t("banks"), href: "/banks" },
        { name: title, href: `/banks/${category}` },
      ]}
